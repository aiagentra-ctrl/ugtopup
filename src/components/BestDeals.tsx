...
const templateRenderers: Record<string, React.FC<{ offer: any }>> = {
  animated_banner: OfferBanner,
  homepage_highlight: OfferHighlight,
  seasonal: OfferSeasonal,
  daily_deal: OfferDailyDeal,
};

// Simple timer for badge-type and fallback offers
function OfferTimer({ offer }: { offer: any }) {
  const [time, setTime] = useState(
    offer.timer_end_date ? getTimeLeft(offer.timer_end_date) : getFallbackTime()
  );

  useEffect(() => {
    const timer = setInterval(() => {
      setTime(offer.timer_end_date ? getTimeLeft(offer.timer_end_date) : getFallbackTime());
    }, 1000);
  ...
  return (
    <section className="py-12 md:py-16 bg-background">
      <div className="container mx-auto px-4 space-y-8">
        {offers.map((offer) => {
          const template = offer.design_template || "badge";
          const Renderer = templateRenderers[template];

          // For templates with dedicated components
          if (Renderer) {
            return <Renderer key={offer.id} offer={offer} />;
          }

          // Default "badge" template — simple centered text + timer (original style)
          return (
            <div key={offer.id} className="flex flex-col items-center justify-center space-y-6 text-center cursor-pointer"
              onClick={() => offer.product_link && window.location.assign(offer.product_link)}
            >
              <h2 className="flex items-center justify-center gap-3 text-2xl md:text-4xl font-bold tracking-wider">
                ...
                <span className="text-primary uppercase" style={{ letterSpacing: "0.1em" }}>{offer.title}</span>
