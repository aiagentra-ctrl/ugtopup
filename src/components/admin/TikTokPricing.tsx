...
export const TikTokPricing = () => {
  ...
  const fetchPrices = async () => {
    setLoading(true);
    try {
      const data = await fetchAllGamePrices('tiktok');
      setPrices(data);
  ...
  useEffect(() => {
    const channel = supabase
      .channel('admin-tiktok-prices')
      .on('postgres_changes', {
        event: 'UPDATE',
        schema: 'public',
        table: 'game_product_prices',
        filter: 'game=eq.tiktok'
      }, (payload) => {
  ...
  const handleQuantityChange = (id: string, value: string) => {
    setRowStates(prev => ({
      ...prev,
      [id]: { ...prev[id], quantity: value, saved: false, error: false }
    }));
  };
  ...
  const handlePriceChange = (id: string, value: string) => {
    setRowStates(prev => ({
      ...prev,
      [id]: { ...prev[id], price: value, saved: false, error: false }
    }));
  };
  ...
  const handleSaveRow = async (id: string) => {
    ...
    try {
      ...
      setRowStates(prev => ({
        ...prev,
        [id]: { ...prev[id], saving: false, saved: true, originalQuantity: quantity, originalPrice: price }
      }));
  ...
  const handleSaveAll = async () => {
    const changedRows = prices.filter(p => {
      ...
      return state && (
        parseInt(state.quantity) !== state.originalQuantity ||
        parseFloat(state.price) !== state.originalPrice
      );
    });
  ...
  const hasChanges = (id: string) => {
    ...
    return parseInt(state.quantity) !== state.originalQuantity ||
           parseFloat(state.price) !== state.originalPrice;
  };
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <span className="text-lg">🎵</span> TikTok Coins Packages
          </CardTitle>
        </CardHeader>
        <CardContent>
              ...
              <div key={pkg.id} className="grid grid-cols-12 gap-2 items-center p-2 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors">
                ...
                <div className="col-span-2">
                  <Button
                    ...
                    onClick={() => handleSaveRow(pkg.id)}
                    disabled={rowStates[pkg.id]?.saving || !hasChanges(pkg.id)}
                    className="h-8 w-full text-xs"
                  >
                    {rowStates[pkg.id]?.saving ? <Loader2 className="h-3 w-3 animate-spin" /> : 'Save'}
                  </Button>
                </div>
              </div>
            ))}
